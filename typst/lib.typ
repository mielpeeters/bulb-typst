...
#let dither(
  ...
  assert(size == none or (type(size) == int and size > 0), message: "size must be none or a positive integer, got " + repr(size))
  ...
  if mode == "palette" {
    ...
    assert(
      ...
      message: "unknown palette-method: " + repr(palette-method) + ", expected one of: " + repr(_palette-methods.keys()),
    )
  }
